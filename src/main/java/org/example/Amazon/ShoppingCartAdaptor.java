...
public class ShoppingCartAdaptor implements ShoppingCart {
  ...
  public void add(Item item) {
    connection.withSql(
        () -> { // Executes SQL operations within the database connection
          try (var ps =
              connection
                  .getConnection()
                  .prepareStatement(
                      "insert into shoppingcart (name, type, quantity, priceperunit) values (?,?,?,?)")) { // Prepares the SQL query to insert a new invoice
        ...
        () -> { // Executes SQL operations within the database connection
          try (var ps =
              connection
                  .getConnection()
                  .prepareStatement(
                      "select * from shoppingcart")) { // Prepares the SQL query to select all
            // invoices
            final var rs = ps.executeQuery(); // Executes the query and stores the result set

            List<Item> ShoppingCart =
                new ArrayList<>(); // Creates a list to store all retrieved invoices
            while (rs.next()) { // Iterates through each row in the result set
              ShoppingCart.add(
                  new Item(
                      ...
                      rs.getInt("quantity"),
                      rs.getDouble(
                          "priceperunit"))); // Creates a new Invoice object and adds it to the list
            }
  ...
  public int numberOfItems() {
    return connection.withSql(
        () -> {
          try (var ps =
              connection.getConnection().prepareStatement("select count(*) from shoppingcart")) {
